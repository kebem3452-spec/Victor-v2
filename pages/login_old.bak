"""
VICTOR V2 — pages/login.py v3
Session via COOKIE NAVIGATEUR
"""
...
def render_phone_input_login():
    ...
    with col2:
        if AFRICA_CODES[selection] == "autre":
            ...
            st.text_input("Indicatif", value=prefix, disabled=True,
                           key="login_prefix_auto")
            num = st.text_input("Numéro", placeholder="77 000 00 00",
                                 key="login_num_auto")
...
def afficher_login():
    ...
    # ── PRIORITÉ 1 : session active en mémoire ──
    if st.session_state.get("connecte"):
        st.switch_page("5_interface_web.py")
        return

    # ── PRIORITÉ 2 : cookie présent → laisser interface gérer ──
    cookie_val = cookies.get(COOKIE_NAME)
    if cookie_val and ":" in cookie_val:
        st.switch_page("5_interface_web.py")
        return

    # ── PRIORITÉ 3 : ancien format URL ?t= ?p= → laisser interface gérer ──
    ...
    # ── PRIORITÉ 4 : formulaire de connexion ──
    st.markdown("""<style>
    ...
    with col2:
        ...
        if st.button("🚀 Se connecter", use_container_width=True, type="primary"):
            if len(telephone) < 8 or not mot_de_passe:
                ...
                if result["succes"]:
                    ...
                    # Cookie navigateur — valable 30 jours
                    cookies.set(
                        COOKIE_NAME,
                        f"{abonne['telephone']}:{abonne['session_token']}",
                        max_age=60*60*24*30
                    )
        ...
        wa_msg = ("Bonjour, j'ai perdu mon mot de passe Victor V2. "
                  "Mon numéro est : " + telephone)
        wa_url = f"https://wa.me/221762641751?text={wa_msg.replace(' ', '%20')}"
        st.link_button("🔑 Mot de passe oublié ?", wa_url,
